...
const PetController = {
    ...
    async atualizar(req, res) {
        try {
            if (!req.userId) return res.status(401).json({ error: 'Usuário não autenticado.' });
            if (req.userPerfil !== 'Doador') {
                return res.status(403).json({ error: 'Apenas doadores podem atualizar pets.' });
            }
            ...
            const { Nome, Tipo, Porte, Genero, Idade, Descricao, Img, Cidade, Estado } = req.body;
            const updates = {};
            ...
            if (Cidade && Estado) {
                const [local] = await Localizacao.findOrCreate({ where: { Cidade, Estado } });
                updates.idLocal = local.idLocal;
            }
