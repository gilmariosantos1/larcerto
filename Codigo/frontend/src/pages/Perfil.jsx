...
export default function Perfil() {
  ...
  useEffect(() => {
    ...
    async function fetchDados() {
      try {
        const petsRes = user?.Perfil === 'Doador'
          ? await api.get('/pets').catch(() => ({ data: [] }))
          : null
        const adocoesRes = user?.Perfil === 'Adotante'
          ? await api.get('/adocoes/minhas').catch(() => ({ data: [] }))
          : null

        if (user?.Perfil === 'Doador' && petsRes) {
          const meus = petsRes.data.filter(p => p.doador?.idPessoa === user?.idPessoa)
          setMeusPets(meus)
        }

        if (user?.Perfil === 'Adotante' && adocoesRes) {
          setMinhasAdocoes(adocoesRes.data)
        }
      } catch (err) {
        console.error(err)
      } finally {
